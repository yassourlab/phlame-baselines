...
def run_with_params(train, test_x, train_subject_ids, model_name, param_grid, run_type, partial_output_dir):
    """
    ...
    best_params = grid_search.cv_results_['params'][grid_search.best_index_].copy()
    ...
    params_search_df = pd.concat([
        params_df,
        fold_scores,
        cv_results[['mean_test_score', 'std_test_score', 'mean_fit_time', 'std_fit_time']]
    ], axis=1)
...
def run_default(train, test_x, train_subject_ids, model_name, partial_output_dir):
...
def run_optimized(train, test_x, train_subject_ids, model_name, partial_output_dir):
...
if __name__ == '__main__':
    ...
    default_output_dir = run_default(train, test_x, train_subject_ids, model_name, output_dir)
    
    # Train with parameter search
    print("Training with parameter search...")
    optimized_output_dir = run_optimized(train, test_x, train_subject_ids, model_name, output_dir)
