...
class DeepMicrobiome(object):
    def __init__(self, X_train, X_test, y_train, y_test,data_name, out_dir, seed=17):
    ...
    def ae(self, dims=[50], epochs=2000, batch_size=100, verbose=2, loss='mean_squared_error', latent_act=False,
        ...
        self.autoencoder, self.encoder = DNN_models.autoencoder(dims, act=act, latent_act=latent_act,
        ...
        # fit model
        self.history = self.autoencoder.fit(X_inner_train, X_inner_train, epochs=epochs, batch_size=batch_size,
                                            callbacks=callbacks,
    ...
    def vae(self, dims=[10], epochs=2000, batch_size=100, verbose=2, loss='mse', output_act=False, act='relu',
        ...
        warm_up_cb = LambdaCallback(
            on_epoch_end=lambda epoch, logs: [warm_up(epoch)])  # , print(epoch), print(K.get_value(beta))])
        ...
        # create vae model
        self.vae, self.encoder, self.decoder = DNN_models.variational_AE(dims, act=act, recon_loss=loss,
                                                                         output_act=output_act, beta=beta)
        self.vae.summary()
        ...
        # fit
        self.history = self.vae.fit(X_inner_train, epochs=epochs, batch_size=batch_size, callbacks=callbacks,
                                    verbose=verbose, validation_data=(X_inner_test, None))
    ...
    def cae(self, dims=[32], epochs=2000, batch_size=100, verbose=2, loss='mse', output_act=False, act='relu',
        ...
        self.X_train = np.column_stack(
            (self.X_train, np.zeros((self.X_train.shape[0], enlargedDim - self.X_train.shape[1]))))
        self.X_test = np.column_stack(
            (self.X_test, np.zeros((self.X_test.shape[0], enlargedDim - self.X_test.shape[1]))))
        ...
        # create cae model
        self.cae, self.encoder = DNN_models.conv_autoencoder(dims, act=act, output_act=output_act, rf_rate=rf_rate,
                                                             st_rate=st_rate)
        self.cae.summary()
        if no_trn:
            return
        ...
        # fit
        self.history = self.cae.fit(X_inner_train, X_inner_train, epochs=epochs, batch_size=batch_size,
    ...
    def saveLossProgress(self):
        ...
        if not os.path.exists(self.data_dir + "results/"):
            os.makedirs(self.data_dir + "results/")
        ...
        if 'recon_loss' in self.history.history:
            ...
            plt.ylabel('loss')
            plt.xlabel('epoch')
            plt.legend(['train loss', 'val loss', 'recon_loss', 'val recon_loss', 'kl_loss', 'val kl_loss'],
                       loc='upper right')
            plt.savefig(self.data_dir + "results/" + figureName + '.png')
            plt.close()
    ...
    #                                 ])
    # load_data.add_argument("-cd", "--custom_data", help="filename for custom input data under the 'data' folder",
    #                        type=str, )
    # load_data.add_argument("-cl", "--custom_data_labels",
    #                        help="filename for custom input labels under the 'data' folder", type=str, )
    # load_data.add_argument("-p", "--data_dir", help="custom path for both '/data' and '/results' folders", default="")
    # load_data.add_argument("-dt", "--dataType",
    #                        help="Specify data type for numerical values (float16, float32, float64)",
    ...
    # exp_design.add_argument("-r", "--repeat",
    #                         help="repeat experiment x times by changing random seed for splitting data",
    #                         default=5, type=int)
    #
    # # classification
    # classification = parser.add_argument_group('Classification')
    # classification.add_argument("-f", "--numFolds",
    #                             help="The number of folds for cross-validation in the tranining set",
    #                             default=5, type=int)
    ...
    # rl.add_argument("--save_rep", help="write the learned representation of the training set as a file",
    #                 action='store_true')
    #
    ...
    # VAE.add_argument("--vae_warmup_rate",
    #                  help="warm-up rate which will be multiplied by current epoch to calculate current beta",
    #                  default=0.01, type=float)
    #
    # ## CAE only
    # CAE = parser.add_argument_group('CAE-specific arguments')
    # CAE.add_argument("--rf_rate", help="What percentage of input size will be the receptive field (kernel) size? [0,1]",
    #                  type=float, default=0.1)
    # CAE.add_argument("--st_rate",
    #                  help="What percentage of receptive field (kernel) size will be the stride size? [0,1]", type=float,
    #                  default=0.25)
    #
    # # other options
    # others = parser.add_argument_group('other optional arguments')
    # others.add_argument("--no_trn", help="stop before learning representation to see specified autoencoder structure",
    #                     action='store_true')
    ...
    #     if args.vae:
    #         dm.vae(dims=[int(i) for i in args.dims.split(',')], act=args.act, epochs=args.max_epochs, loss=args.aeloss,
    #                output_act=args.ae_oact,
    #                patience=25 if args.patience == 20 else args.patience, beta=args.vae_beta, warmup=args.vae_warmup,
    #                warmup_rate=args.vae_warmup_rate, no_trn=args.no_trn)
    #     if args.cae:
    #         dm.cae(dims=[int(i) for i in args.dims.split(',')], act=args.act, epochs=args.max_epochs, loss=args.aeloss,
    #                output_act=args.ae_oact,
